  ...
  return (
    <>
      ...
      >
        ...
        <span style={{ width: '30%', fontWeight: 'Bold', textAlign: 'center' }}>Authors</span>
        <span style={{ width: '20%', fontWeight: 'Bold', textAlign: 'center' }}>Category</span>
        <span style={{ width: '10%', fontWeight: 'Bold', textAlign: 'center' }}>Actions</span>
      </div>
      <hr style={{ margin: '0' }} />
    </>
  );
...
const ResearchIncentiveModal = ({ show, onClose, applicationId, isEditMode, fetchIncentives }) => {
  ...
  const handleCurPageCategory = (newCategoryId, data, formData, selectedFiles) => {
  ...
  return (
    <Modal show={show} onHide={onClose} dialogClassName="custom-research-incentive-modal">
        ...
        .then((res) => {
          if (res.data.success) {
            const category = res.data.data.find(
              (category) => category.category_id === data.applications[0].category_id,
            );
            setCategoryDescription(category ? category.category_description : 'Unknown');
          }
        })
        .catch((error) => {
  ...
  return (
    <div style={{ fontSize: '14px' }}>
      <div
        ...
        onClick={() => handleOpenIncentiveModal(data.applications[0].application_id)}
      >
        <span
          title={data.title}
          style={{
        ...
        <span style={{ width: '20%', textAlign: 'center' }}>{categoryDescription}</span>
        <span style={{ width: '10%', textAlign: 'right' }}>
          <span
            ...
            onClick={(e) => {
              e.stopPropagation();
              handleOpenIncentiveModal(data.applications[0].application_id, true);
            }}
          >
...
const IncentivesPanel = () => {
      ...
      .then((res) => {
        if (res.data.success) {
          const formattedData = res.data.data.map((incentives) => {
            return {
              ...incentives,
              authors: incentives.authors.map((author) => author.author_name).join(', '),
            };
          });
  ...
  return (
    <div className="pt-3 px-0" style={{ width: '90%' }}>
      ...
      <Tabs id="controlled-tab-notifications" activeKey={key} onSelect={(k) => setKey(k)}>
