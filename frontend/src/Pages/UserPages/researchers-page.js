...
const TabHeader = () => {
  return (
    <>
      ...
      >
        <span style={{ width: '20%', marginLeft: '2%', fontWeight: 'Bold' }}>Name</span>
        <span style={{ width: '10%', fontWeight: 'Bold', textAlign: 'center' }}>Department</span>
        <span style={{ width: '15%', fontWeight: 'Bold', textAlign: 'center' }}>Role</span>
        <span style={{ width: '25%', fontWeight: 'Bold', textAlign: 'center' }}>Email Address</span>
        <span style={{ width: '15%', fontWeight: 'Bold', textAlign: 'center' }}>
          Account Status
        </span>
        <span style={{ width: '15%', fontWeight: 'Bold', textAlign: 'center' }}>Actions</span>
      </div>
      <hr style={{ margin: '0' }} />
    </>
  );
    ...
    <>
      ...
      >
        <div style={{ display: 'flex', width: '95%', alignItems: 'center' }}>
          ...
          <div style={{ width: '10%', textAlign: 'center' }}>{item.citations}</div>
          <div style={{ width: '20%', textAlign: 'center' }}>{format_date(item.year)}</div>
        </div>
      </div>
      <hr style={{ margin: '0' }} />
    </>
...
const CoAuthorCard = ({ data }) => {
  return (
    <div className="d-flex align-items-center my-2">
      <Image src={require('../../assets/images.jpg')} roundedCircle height={32} />
      <div className="ml-2">
  ...
  return (
    <div style={{ fontSize: '14px' }}>
      ...
      >
        ...
        <span style={{ width: '14%', textAlign: 'center' }}> {data.department} </span>
        <span style={{ width: '15%', textAlign: 'center' }}> {data.role} </span>
        <span style={{ width: '25%', textAlign: 'center' }}> {data.email} </span>
          ...
          <div style={{ overflowY: 'scroll', height: '60vh' }}>
            {researchers.length > 0 ? (
              researchers.map((item) => (
                <ResearcherItem key={item.user_id} data={item} val={change} trigger={setChange} />
              ))
            ) : (
